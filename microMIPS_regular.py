...
#---------------------INC---------------------------
def BGTZC_J_REFORMAT(expression):
	pass
	#formating var expression to easily access important values as dictionary
#---------------------INC---------------------------
...
if __name__ == '__main__':
	...
	while input_Phase and int(address_hex, 16) < int("2000", 16):
		ins_Input = {}
		ins_Input["inst_String"] = input(address_hex + " ").upper()
	...
	while not if_insError and not if_paramError and count < len(ins_String):
		...
		if ins_String[count]["inst_String"].split(" ")[0] in ins_List:
			...
			if if_paramError:
				...
				ins_Format = type_Form[ins_String[count]["ins_Num"]](ins_String[count]["inst_String"])
